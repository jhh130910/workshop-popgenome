...
# Population genetic analysis using R and PopGenome
...
- [Introduction to Popgenome](https://github.com/tonig-evo/workshop-popgenome/blob/master/An_introduction_to_the_PopGenome_package.pdf)
- [Popgenome manual](https://github.com/tonig-evo/workshop-popgenome/blob/master/PopGenome.pdf)
- [PopGenome and vcf files](https://github.com/tonig-evo/workshop-popgenome/blob/master/Whole_genome_analyses_using_VCF_files.pdf)

To work on the cluster, copy the necessary files into your working directory (e.g. /global/work/kurs01-11/)

```bash 
mkdir PopGenome
cd PopGenome
wget https://github.com/tonig-evo/workshop-popgenome/raw/master/PopGenome_data.zip
unzip PopGenome_data.zip
rm PopGenome_data.zip
```

### Loading R and installing PopGenome
...
tutorial, please make sure that your R is loaded, the R working directory is set
correctly and you have all the packages installed. Please choose a CRAN server without [https].
...
Editor (e.g. Notepad+) or via the command line (e.g. more or nano) before
...
beforehand. For this modules have to be run. An overview about available modules cna be ontained with show.slots().
Note that module **Fst** has to be executed with **F_st**. The statistic Tajima’s D is part of
...
7.How many singletons are in the dataset? 

(see also **An_introduction_to_the_PopGenome_package.pdf**, section 3.1)

8.What is the difference between *region.data* and *region.stats*? 

(see also **Whole_genome_analyses_using_VCF_files.pdf**, section 11 and 12)
...
ggsave("plot_sfs.pdf", width = 5, height = 5)
```
Additional aspects
==================
